...
Figure~\ref{fig:two_views}(c) draws it. On the left stands the library. A work
has an address $(u, v)$: its focus $u$ in $\mathcal{U}$, its connotation $v$ in
$\mathcal{V}$. The two run apart---which floor, and where on the floor. On the
right lies the content data, where an embedding $\Phi$ sends the address to a
point $\mathbf{x} = \Phi(u, v)$. There the two no longer run apart: each floor
$\Phi(u, \cdot)$ curves into a galaxy, and each rabbit hole $\Phi(\cdot, v)$
into a thread running from galaxy to galaxy. One representation, two views.
...
bundle---the spaces that lose the least, principal in the sense of PCA, and
essentially unique under mild regularity. We are free in $D_\mathcal{V}$; two
dimensions serve exploration well, since a floor one can walk is a plane.
