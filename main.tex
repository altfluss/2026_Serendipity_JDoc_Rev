...
\begin{abstract}\noindent
    ...
    interface presupposes and none can substitute for---and what is needed is
    a \textit{disentangled} one, separating the denotative from the
    connotative. Once the scope of the data is fixed, an inductive bias
    follows naturally: \textit{optimal transport}. The resulting
    \textit{Optimal Transport Fiber Bundle (OT-FB)} is essentially unique
    among the infinitely many representations fitting the same data, and
    learnable from real data. What the bundle says about rabbit holes: a hole
    is determined not by what two works have in common, but by how one genre
    stands to another as a whole. Neither pairwise affinity nor one more
    label expresses it---a name attaches to one work at a time; a coordinate,
    naming nothing, does. Such signposts are what the seeker needs: in this
    space, what cannot be searched for can be sought.
\end{abstract}
...
\begin{quote}\parindent=0pt\parskip=.5em\noindent
    ...
    rather than interface; realizes that representation in a form both
    unique and learnable from data; and marks a limit of naming: a name
    attaches to one work at a time, while connotation lives in how whole
    domains stand to one another. A coordinate, naming nothing, carries
    it---and by such signposts, what cannot be searched for can be sought.
\end{quote}
