...
encyclopedia of visual art. Here a focus is compound: a style and a
subject together, such as \textit{Impressionist landscape}. Nine
styles---Baroque, Rococo, Romanticism, Realism, Impressionism,
Post-Impressionism, Expressionism, Symbolism, and Art Nouveau---each
pair with two or three subjects among portrait, genre painting,
landscape, and religious painting, giving twenty-six floors of two
hundred fifty paintings each. The content data come from the image
alone: each painting is resized whole to 224 by 224, never cropped,
and embedded by CLIP~\cite{Radford2021,Cherti2023}, a joint
image--text encoder, then normalized to unit norm, centered, and
reduced by PCA to ten dimensions. These embeddings and the
style--subject labels are all the modeling uses. The collection also
carries ArtEmis~\cite{Achlioptas2021}---nine emotions and a written
reason, from a median of five viewers per painting---and they are
held out, as the community tags were.

\paragraph{The learned bundle.}
The twenty-six floors are learned as one OT-FB, as the five genres
were. Figure~\ref{fig:wikiart}(a) shows the manifolds in $\mathcal{X}$:
twenty-six surfaces, packed close, each shading into the next. The
algorithm was told twenty-six names and nothing of how they were made.
Style and subject never entered as such.
...
\paragraph{The genre map.}
Figure~\ref{fig:wikiart}(b) shows the denotation space $\mathcal{U}$.
The floors fall into rows---landscape above, genre painting in the
middle, portrait below---and within each row the order repeats:
Expressionism, Impressionism, Realism, Romanticism, Baroque. Style
runs across, subject runs down. Twenty-six names went in, and this
collection returned two axes.
