...
Throughout, we keep to the simplest case, in which the only descriptors
given are genre and title.

Consider first what descriptors alone can build: works gathered by
genre, one floor to each, and within a floor shelved in title order.
The visitor asks for the jazz floor and finds \textit{Kind of Blue},
and there his intention runs out. Title order says nothing about what a
work is like, so even along the jazz floor he cannot move toward what he
is after; on the classical floor he can only take works down at random.
Past \textit{Kind of Blue}, everything is left to chance.

Suppose now that content data is available---acoustic features,
say---and that each floor is arranged by acoustic similarity. The
arrangement rewards him at once: near \textit{Kind of Blue} stand other
Miles Davis recordings, and other trumpet-led works besides. It works
because the genre is fixed for everyone on the floor, so what acoustic
distance measures there is what the genre leaves open---something of
connotation. Off the floor it measures something else. Looking among the
classics for what sounds like jazz, he may turn up Gershwin; Debussy he
will not, for a solo piano work and a jazz quintet share almost nothing
acoustically. Across floors, acoustic distance is dominated by genre
itself, and carries him not to what he means but to whichever genre
sounds nearest.

Bj\"orneborn's three affordances (Section~\ref{subsec:systems}) name
what the physical library already gives its
explorers~\cite{Bjorneborn20171053}. Restated as requirements on a
representation---and asked to hold for connotation, not only for
denotation---they become three conditions.
...
\begin{itemize}
    ...
    \item \textbf{(R2) Foreseeable in direction:} he can tell what each
          ...
          stays.\footnote{Bj\"orneborn's diversifiability covers
              difference along dimensions of many kinds, including facets a
              system may offer; we take it here in its spatial sense alone.}
    \item \textbf{(R3) Traversable at will:} he can choose his own
          direction and move through the space as he pleases.
\end{itemize}

The two libraries fall short in different ways. The descriptor-based one
affords R1 alone: no direction along a shelf holds anything the visitor
could foresee (R2), and freedom to walk without foresight is wandering
rather than exploration (R3). Arranging floors by acoustic similarity
gains R2 and R3, but within a floor only; across genres, no direction
tells him what will change and what will stay.

That is where disentanglement earns its name. What R2 asks for across
floors is a direction that holds one aspect and varies the other---genre
fixed and connotation moving, or connotation fixed and genre moving. A
space offers such directions only if the two have been pulled apart
within it: only, that is, if the representation is \emph{disentangled}
(Section~\ref{subsec:disentanglement}).

The multi-floor library with rabbit holes is our conceptualization of a
space meeting all three (Figure~\ref{fig:two_views}a). Each floor holds
...
What joins the floors into one space is the rabbit hole. At every point
on every floor one lies underfoot, and it runs vertically through the
whole building, opening onto every floor: step in, and one comes out at
the corresponding position---the same connotation, however distant the
genre.\footnote{For clarity we depict each rabbit hole as
    one-dimensional; in general it may be two- or higher-dimensional.}
From Charlie Parker on the jazz floor, one hole passes Frank Zappa on
the rock/pops floor and opens beside Stravinsky among the classics:
three restless, angular, rhythmically intricate bodies of work,
generically far apart. And because the holes lie everywhere, the leap is
available from wherever the visitor happens to stand (R3).
...
written in no field. Its coordinate $v$ is read from the content data:
the works themselves, and the words that circle them.
